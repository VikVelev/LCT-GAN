...
def latent_gan_experiment(
    ...
    ae = pickle.load(
        open(f"ae_pickles/latent_ae{bottleneck}_{ae_epochs}.pickle", 'rb'))
    # ae = autoencoder
    ...
    # decoded_df = ae.decode(lat_data)
    # decoded_df.to_csv(f"./data/Adult_decoded{bottleneck}_{ae_epochs}.csv", index=False)
    # print(decoded_df)

    # evaluate("./data/Adult.csv", [f"./data/Adult_decoded{bottleneck}_{ae_epochs}.csv"])

    latent_data = lat_data_np  # could be loaded from file.

    sscaler = StandardScaler()
    sscaler.fit(latent_data)

    lat_normalized = sscaler.transform(latent_data)
    gan = LatentGAN(bottleneck, latent_dim=gan_latent_dim)
    gan.fit(lat_normalized, ae.train_data, ae.transformer,
            epochs=gan_epochs, batch_size=gan_batch_size, n_critic=gan_n_critic)
    gan_picklefile = open(
        f"gan_pickles/gan_{gan_latent_dim}_{gan_epochs}_.pickle", 'wb')
    pickle.dump(gan, gan_picklefile)

    # gan = pickle.load(open(f"gan_pickles/gan_{gan_latent_dim}_{gan_epochs}.pickle", 'rb'))

    df = gan.sample(len(lat_data), ae, sscaler)
    df.to_csv("./data/Adult_fake.csv", index=False)
    print(df)

    evaluate("./data/Adult.csv", ["./data/Adult_fake.csv"], ['workclass', 'education', 'marital-status', 'occupation', 'relationship', 'race', 'gender', 'native-country', 'income'])
...
def ae_experiment():
    ...
    for exp in tqdm(experiment_params):
        ...
        ae = LatentTAE(**exp)
        ae.fit(n_epochs=epochs, batch_size=512)
        ...
        try:
            res = [ time_to_train, evaluate(real_path, fake_paths, exp["categorical_columns"], ml=False) ]
        except:
            pass
        print(res)
        results.append(res)
        pf = open(f"./results/ae/{exp['raw_csv_path'].replace('./data/', '').replace('.csv','_results.pickle')}", 'wb')
