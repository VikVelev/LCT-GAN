...
class LatentTAE:
    ...
    def decode(self, latent, batch=False):
        if batch:
            ...
            for l in tqdm(latent):
                ...
                reconstructed = np.asarray([reconstructed.cpu().detach().numpy()])
    ...
    def get_latent_dataset(self, data=None, leave_pytorch_context=False):

        latent_dataset = []
        for d in self.train_data:
            
            latent_dataset.append(self.encode(d).cpu().detach().numpy() if leave_pytorch_context else self.encode(d))
...
class AutoEncoder(object):
    ...
    def encode(self, x):
        datum = torch.from_numpy(x).to(self.device).view(-1, self.input_size).float()
    ...
    def train(self, data, output_dim: int, output_info, epochs=5000, batch_size=1024):
        ...
        for i in tqdm(range(epochs)):
            ...
            loss = self.loss_function(recon_batch, data, input_size=self.input_size)
            loss.backward()
